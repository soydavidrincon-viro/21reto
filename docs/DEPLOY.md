...
El código vive en GitHub: `soydavidrincon-viro/21reto`, rama
`claude/antidoto-app-plan-cj964q`.
...
4. Antes de darle Deploy, abre **Environment Variables** y agrega las dos:

   | Name | Value |
   |---|---|
   | `NEXT_PUBLIC_SUPABASE_URL` | `https://jayzecnedknfichbmsce.supabase.co` |
...
navegador, con su icono, como cualquier app.
...
cd 21reto
git checkout claude/antidoto-app-plan-cj964q
npm install
cp .env.example .env.local     # y pega dentro las dos variables
npm run dev
```
...
|---|---|
| Código | GitHub |
| Base de datos, cuentas y sesiones | Supabase |
| La app servida | Vercel (o tu máquina) |
